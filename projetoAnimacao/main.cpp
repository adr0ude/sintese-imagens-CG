...
#include "../projetoAnimacao/miniaudio.h"

// --- ESTRUTURA DE ÁUDIO ATUALIZADA ---
#define TOTAL_SONS_CENA 8
ma_engine g_soundEngine;
ma_sound g_scene_sounds[TOTAL_SONS_CENA];
int g_current_sound_index = -1;

// Nomes dos arquivos de áudio, começando com 01
const char* g_audio_files[TOTAL_SONS_CENA] = {
    "audio01.wav",
    "audio02.wav",
    "audio03.wav",
    "audio04.wav",
...
// --- FIM DA ESTRUTURA DE ÁUDIO ---

float carroPosY = -10.0f;
GLUquadric* quad;
int cenaAtual = 0;
// Voltamos para 8 cenas no total
int totalCenas = 8; 
// Tempos para todas as 8 cenas (valores do seu código original)
int temposCena[] = {6000, 6000, 17000, 6000, 8000, 8000, 7000, 7000}; 

void initAudio() {
    ma_result result;
    printf("Debug: Inicializando a engine de audio...\n");
    result = ma_engine_init(NULL, &g_soundEngine);
    if (result != MA_SUCCESS) {
        printf("Falha ao inicializar a engine de audio.\n");
        exit(1);
    }

    printf("Debug: Carregando os %d sons de cena...\n", TOTAL_SONS_CENA);
    for (int i = 0; i < TOTAL_SONS_CENA; ++i) {
        result = ma_sound_init_from_file(&g_soundEngine, g_audio_files[i], MA_SOUND_FLAG_DECODE | MA_SOUND_FLAG_NO_PITCH, NULL, NULL, &g_scene_sounds[i]);
        if (result != MA_SUCCESS) {
            printf("Erro ao carregar o som: %s\n", g_audio_files[i]);
        } else {
            ma_sound_set_looping(&g_scene_sounds[i], MA_TRUE);
        }
    }
     printf("Debug: Carregamento de sons concluido.\n");
}

void cleanupAudio() {
    printf("Debug: Liberando recursos de audio...\n");
    for (int i = 0; i < TOTAL_SONS_CENA; ++i) {
        ma_sound_uninit(&g_scene_sounds[i]);
    }
    ma_engine_uninit(&g_soundEngine);
}
...
void atualizarZigZag(int value) {
    ...
    if (zigzagX > 1.0f || zigzagX < -1.0f)
        zigzagDirecao *= -1.0f;
    glutPostRedisplay();
    glutTimerFunc(16, atualizarZigZag, 0);
}
...
void trocarCena(int value) {
  if (g_current_sound_index != -1 && g_current_sound_index < TOTAL_SONS_CENA) {
      ma_sound_stop(&g_scene_sounds[g_current_sound_index]);
      ma_sound_seek_to_pcm_frame(&g_scene_sounds[g_current_sound_index], 0);
  }
  ...
  if (cenaAtual < TOTAL_SONS_CENA) {
      ma_sound_start(&g_scene_sounds[cenaAtual]);
      g_current_sound_index = cenaAtual;
  }
...
void init() {
    ...
    // Toca o som da primeira cena (audio01.wav)
    if (TOTAL_SONS_CENA > 0) {
        ma_sound_start(&g_scene_sounds[0]);
        g_current_sound_index = 0;
    }
}
